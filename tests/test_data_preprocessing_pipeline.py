...
def create_default_orchestrator(label_mapping=None, include_all_stages=True):
    ...
    if include_all_stages:
        ...
        orchestrator.add_stage(
            DataTransformationStage(config={"normalize_lowercase": True, "normalize_trim": True})
        )
...
def test_individual_stages():
    ...
    result = DataTransformationStage(
        config={"normalize_lowercase": True, "normalize_trim": True}
    ).process(result.processed_data if result and result.processed_data else None)
    print_stage_result(result)
    data = get_processed_data(result)
    if data:
