...
def calculate_metrics(y_true, y_pred, y_conf):
    ...
    avg_confidence_incorrect = (
        np.mean(np.array(y_conf)[~correct_mask]) if np.any(~correct_mask) else 0
    )
...
def evaluate_model(
    ...
    test_file: str = "data/datasets/raw/classification_test.jsonl",
    output_file: str = "models/intent_classifier/evaluation_report.json",
):
    ...
    # Confusion matrix
    print_confusion_matrix(np.array(metrics["confusion_matrix"]), CATEGORIES)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--test-file",
        default="data/datasets/raw/classification_test.jsonl",
    ...
    evaluate_model(
        model_path=args.model_path, test_file=args.test_file, output_file=args.output_file
    )
