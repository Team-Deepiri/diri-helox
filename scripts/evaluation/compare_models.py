...
def compare_models(model_paths: List[str]):
    ...
    if all("eval_report" in m and "metrics" in m["eval_report"] for m in models_data):
        best_model = max(
            models_data, key=lambda m: m["eval_report"]["metrics"]["overall"]["accuracy"]
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--snapshot", action="store_true", help="Create a versioned snapshot of the model"
    )
    parser.add_argument("--version", type=str, default=None, help="Version name for snapshot")
