...
model.eval()
# use single sample to avoid ONNX batch shape issues
sample = tokenizer(["Write unit tests for my API"], return_tensors="pt", padding=True, truncation=True, max_length=64)
print("     Done.\n")
...
mlflow_model = AutoModelForSequenceClassification.from_pretrained(MODEL_NAME, num_labels=3, ignore_mismatched_sizes=True)
...
with torch.no_grad():
    torch.onnx.export(
        model, (sample["input_ids"], sample["attention_mask"]), onnx_path,
...
# ── Inference speed ───────────────────────────────────────────
ort_in = {"input_ids": sample["input_ids"].numpy(), "attention_mask": sample["attention_mask"].numpy()}

with torch.no_grad():
    t0 = time.perf_counter()
    for _ in range(RUNS): mlflow_model(**sample)
    pt_ms = (time.perf_counter()-t0)/RUNS*1000

t0 = time.perf_counter()
for _ in range(RUNS): sess.run(None, ort_in)
...
# ── QLoRA insertion ───────────────────────────────────────────
lora_cfg = LoraConfig(task_type=TaskType.SEQ_CLS, r=8, lora_alpha=16, target_modules=["q_lin","k_lin"], lora_dropout=0.1)
...
print(f"  {'Inference speed':<28} {pt_ms:.1f} ms{'':<18} {onnx_ms:.1f} ms ({pt_ms/onnx_ms:.1f}x faster)")
...
print(f"  → Export to ONNX after fine-tuning is done, for production serving ({pt_ms/onnx_ms:.1f}x faster)")
print(SEP)

os.remove(pt_path); os.remove(onnx_path)
