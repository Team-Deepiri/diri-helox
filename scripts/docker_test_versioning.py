...
def test_versioning_in_docker():
    ...
    try:
        ...
        manager = DatasetVersionManager(db_url=f"sqlite:///{db_path}", storage_backend="local")
        ...
        comparison = manager.compare_versions("docker_lease_abstraction_test", "1.0.0", "1.0.1")
        ...
        print(f"   Total versions: {health['summary']['total_versions']}")
        ...
        print("   - Successfully compared version changes")
...
def test_cli_in_docker():
    ...
    try:
        ...
        os.system(
            "cd /app/diri-helox && python scripts/dataset_versioning_cli.py --help > /dev/null 2>&1"
        )
        ...
        print(
            "   3. python scripts/dataset_versioning_cli.py create --name test --type lease_abstraction --path data/samples/lease_abstraction_v1 --summary 'Docker CLI test'"
        )
