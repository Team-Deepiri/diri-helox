...
class RAGAwareTrainingIntegrator:
        ...
        if self.rag_pipeline:
            try:
                ...
                if hasattr(self.rag_pipeline, "retrieve"):
                    ...
                    hasattr(self.rag_pipeline, "is_available") and self.rag_pipeline.is_available()
                ):
                    # RAG bridge
                    results = self.rag_pipeline.retrieve(
                        query=query,
                ...
                for r in results:
                    if isinstance(r, dict):
                        chunk_text = (
                            r.get("challenge_text") or r.get("content") or r.get("text") or ""
                        )
                    else:
                        chunk_text = str(r)
    ...
    def create_retrieval_conditioned_prompt(
        ...
        mask = [0] * len(context_instruction_ids) + [1] * (
            len(token_ids) - len(context_instruction_ids)
        )
