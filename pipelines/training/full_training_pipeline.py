...
class FullTrainingPipeline:
    ...
    def load_and_prepare_data(self):
        ...
        if "validation_dataset_path" in self.config:
            val_dataset = load_dataset(
                "json", data_files=self.config["validation_dataset_path"], split="train"
            )
    ...
    def setup_model(self):
        ...
        layer_type_str = self.config.get(
            "layer_type", "task"
        )  # "domain" | "task" | "user" | "tenant"
        layer_name = self.config.get(
            "layer_name", "default"
        )  # e.g., "b2b", "classification", "user_123"
