...
class StatisticalValidator:
    ...
    def detect_outliers_isolation_forest(
        data: np.ndarray, contamination: float = 0.1, random_state: Optional[int] = 42
    ) -> Tuple[np.ndarray, Dict[str, Any]]:
        """
    ...
    def kolmogorov_smirnov_test(
        data: np.ndarray, distribution: str = "norm"
    ...
    def shapiro_wilk_test(
        data: np.ndarray, max_sample_size: int = 5000, random_state: Optional[int] = None
    ) -> Tuple[float, float, Dict[str, Any]]:
        """
...
class QualityChecker:
    ...
    def _check_consistency(self, df: pd.DataFrame) -> List[QualityMetric]:
        ...
        for col in df.columns:
            ...
            if len(col_types) > 1:
                ...
                type_issues.append({"column": col, "types": [str(t) for t in col_types]})
            ...
            for field_name, field_spec in schema["fields"].items():
                ...
                if "type" in field_spec:
                    ...
                    if not self._type_matches(expected_type, actual_dtype):
                        errors.append(
                            f"Type mismatch for {field_name}: expected {expected_type}, got {actual_dtype}"
                        )
    ...
    def _check_accuracy(
        self, df: pd.DataFrame, reference_data: Optional[Union[pd.DataFrame, List[Dict]]] = None
    ) -> List[QualityMetric]:
        ...
        for col in numeric_cols:
            ...
            outliers_iqr, iqr_details = self.stat_validator.detect_outliers_iqr(
                col_array, multiplier=self.config.iqr_multiplier
            )
            ...
            metrics.append(
                QualityMetric(
                    ...
                    details={
                        ...
                        **{
                            k: v
                            for k, v in iqr_details.items()
                            if k not in ["outlier_count", "outlier_percentage"]
                        },
                    },
                )
            )
            ...
            outliers_zscore, zscore_details = self.stat_validator.detect_outliers_zscore(
                col_array, threshold=self.config.zscore_threshold
            )
            ...
            metrics.append(
                QualityMetric(
                    ...
                    details={
                        ...
                        **{
                            k: v
                            for k, v in zscore_details.items()
                            if k not in ["outlier_count", "outlier_percentage"]
                        },
                    },
                )
        ...
        for col in numeric_cols:
            ...
            try:
                ...
                if (
                    sw_details.get("original_size", len(col_data))
                    > self.config.shapiro_wilk_sample_size
                ):
                    sw_details["warning"] = (
                        f"Test limited to {self.config.shapiro_wilk_sample_size} samples"
                    )
        ...
        if schema and "foreign_keys" in schema:
            ...
            for fk_name, fk_spec in foreign_keys.items():
                ...
                if fk_name in df.columns:
                    ...
                    integrity_score = (
                        1.0 if null_count == 0 else max(0.0, 1.0 - (null_count / len(df)))
                    )
    ...
    def _calculate_dimension_scores(self, metrics: List[QualityMetric]) -> Dict[str, float]:
        ...
        for dimension in [
            ...
            dimension_metrics = [
                m
                for m in metrics
                if m.dimension == dimension and not m.metric_name.endswith("_error")
            ]
    ...
    def _generate_recommendations(
        self, dimension_scores: Dict[str, float], metrics: List[QualityMetric]
    ) -> List[str]:
        """
...
class QualityCheckStage:
    ...
    def process(self, data: Any) -> Any:  # Returns StageResult from base module
        ...
        try:
            ...
            return StageResult(
                success=False, error=f"Quality check failed: {str(e)}", stage_name="quality_check"
            )
    ...
    def validate(self, data: Any) -> Any:  # Returns ValidationResult from base module
        ...
        try:
            ...
            # Run quality check
            quality_report = self.quality_checker.check_quality(
                data=actual_data, dataset_id=self.dataset_id, schema=self.schema
            )
