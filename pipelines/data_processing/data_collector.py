...
class DataCollector:
    ...
    def collect_from_directory(
        ...
        # Remove duplicates
        unique_texts = self.cleaner.remove_duplicates([item["text"] for item in collected_texts])
        collected_texts = [item for item in collected_texts if item["text"] in unique_texts]

        # Save to JSONL
        self._save_jsonl(collected_texts, output_file)

        logger.info(f"Collected {len(collected_texts)} documents from {source_dir}")
    ...
    def collect_from_jsonl(
        ...
        logger.info(f"Collected {len(collected_texts)} documents from {source_file}")
