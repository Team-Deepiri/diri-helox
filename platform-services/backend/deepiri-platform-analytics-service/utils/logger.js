...
const logger = winston.createLogger({
  ...
  format: logFormat,
  defaultMeta: { service: 'platform-analytics-service' },
  transports: [
    new winston.transports.File({
      filename: path.join(logsDir, 'platform-analytics-service-error.log'),
      level: 'error',
      maxsize: 5242880,
      maxFiles: 5,
    }),
    new winston.transports.File({
      filename: path.join(logsDir, 'platform-analytics-service-combined.log'),
      maxsize: 5242880,
      maxFiles: 5,
    }),
  ],
