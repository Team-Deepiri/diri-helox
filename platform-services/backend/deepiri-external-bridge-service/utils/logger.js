...
const logger = winston.createLogger({
  ...
  format: logFormat,
  defaultMeta: { service: 'external-bridge-service' },
  transports: [
    new winston.transports.File({
      filename: path.join(logsDir, 'external-bridge-service-error.log'),
      level: 'error',
      maxsize: 5242880,
      maxFiles: 5,
    }),
    new winston.transports.File({
      filename: path.join(logsDir, 'external-bridge-service-combined.log'),
      maxsize: 5242880,
      maxFiles: 5,
    }),
  ],
